  ...
  inputObjects = bind_rows(
    ...
    expectsInput(objectName = "fireIgnitionProb", objectClass = "RasterLayer",
                 desc = paste("Raster of expected no. of ignitions (lambda in a poisson ditribution). Optional.",
                              "If not present, will first look for 'fireSense_IgnitionPredicted' raster. If this is ",
                              "not found 'noStartPix' is ued to randomly start a given number of fires in the landscape")),
...
doEvent.FavierFireSpread = function(sim, eventTime, eventType, debug = FALSE) {
  switch(
    eventType,
    init = {
      ## Initialise module
      ...
      sim <- scheduleEvent(sim, P(sim)$fireInitialTime, "FavierFireSpread",
    ...
    doFireSpread = {
      ...
      sim <- scheduleEvent(sim, time(sim) + 1, "FavierFireSpread",
                           "doFireSpread", eventPriority = 2.5)  ## always schedule fire
    },
